namespace UntitledGames.Transforms
{
    ...
    public class TransformProEditorGadgets : ScriptableObject, ISerializationCallbackReceiver
    {
        ...
        /// </summary>
        public static bool RepaintAll { get { return TransformProEditorGadgets.repaintAll; } set { TransformProEditorGadgets.repaintAll = value; } }

        public static bool RepaintInspector { get { return TransformProEditorGadgets.repaintInspector; } set { TransformProEditorGadgets.repaintInspector = value; } }
        ...
        public static void Unload()
        {
            if (TransformProEditorGadgets.instance != null)
            {
                SceneView.duringSceneGui -= TransformProEditorGadgets.instance.OnSceneGUI;
        ...
        private void OnSceneGUI(SceneView sceneView)
        {
            ...
            {
                foreach (ITransformProGadgetSceneHandles gadget in this.GadgetsSceneHandles.Where(gadget => gadget.Enabled))
                {
            ...
            float x = (SceneView.currentDrawingSceneView.camera.pixelRect.width / EditorGUIUtility.pixelsPerPoint) - (100 - TransformProEditorGadgets.Padding);
#else
            float x = SceneView.currentDrawingSceneView.camera.pixelRect.width - (100 - TransformProEditorGadgets.Padding);
#endif
            float y = 5;
            float width = 90;
            ...
            if ((e.rawType == EventType.MouseDown) || (e.rawType == EventType.MouseUp))
            {
                if (!useEvent)
                {
                    useEvent = this.GadgetsSceneClick
                                   ...
                                   .Any(gadget => gadget.SceneClick(sceneView, this, e.mousePosition, e.rawType == EventType.MouseDown));
                    if (useEvent)
                    {
