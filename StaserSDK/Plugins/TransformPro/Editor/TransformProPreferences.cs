namespace UntitledGames.Transforms
{
    ...
    public class TransformProPreferences : EditorWindow
    {
        ...
        private static TransformProClipboard[] clipboards = new TransformProClipboard[TransformProPreferences.ClipboardCount];
        ...
        public static TransformProClipboard Clipboard { get { return TransformProPreferences.GetClipboard(TransformProPreferences.SelectedClipboard); } }
        ...
        public static NudgeSettingsPosition NudgesPosition { get { return TransformProPreferences.nudgesPosition ?? (TransformProPreferences.nudgesPosition = new NudgeSettingsPosition()); } }

        public static NudgeSettingsRotation NudgesRotation { get { return TransformProPreferences.nudgesRotation ?? (TransformProPreferences.nudgesRotation = new NudgeSettingsRotation()); } }
        ...
        public static int SelectedClipboard
        {
            ...
            set
            {
                TransformProPreferences.selectedClipboard = Math.Max(0, Math.Min(TransformProPreferences.ClipboardCount - 1, value));
        ...
        public static TransformProClipboard GetClipboard(int index)
        {
            ...
            {
                Debug.LogError(string.Format("[<color=red>TransformPro</color>] Clipboard index '' out of bounds. 10 editor clipboards are available.", index));
                return null;
            }
        ...
        public static void Load()
        {
            ...
            TransformProPreferences.snapPositionGrid = TransformProPreferences.GetVector3("snapPositionGrid", Vector3.one);
            TransformProPreferences.snapPositionOrigin = TransformProPreferences.GetVector3("snapPositionOrigin", Vector3.zero);
            TransformProPreferences.snapRotationGrid = TransformProPreferences.GetVector3("snapRotationGrid", new Vector3(90, 90, 90));
            TransformProPreferences.snapRotationOrigin = TransformProPreferences.GetVector3("snapRotationOrigin", Vector3.zero);
            ...
            for (int clipboardIndex = 0; clipboardIndex < TransformProPreferences.ClipboardCount; clipboardIndex++)
            {
                TransformProClipboard clipboard = new TransformProClipboard
                                                  {
                                                      Position = TransformProPreferences.GetVector3(string.Format("clipboard{0}.Position", clipboardIndex), Vector3.zero),
                                                      Rotation = TransformProPreferences.GetQuaternion(string.Format("clipboard{0}.Rotation", clipboardIndex), Quaternion.identity),
                                                      Scale = TransformProPreferences.GetVector3(string.Format("clipboard{0}.Scale", clipboardIndex), Vector3.one)
                                                  };
        ...
        {
            TransformProPreferences.SetVector3(string.Format("clipboard{0}.Position", TransformProPreferences.SelectedClipboard), TransformProPreferences.Clipboard.Position);
            TransformProPreferences.SetQuaternion(string.Format("clipboard{0}.Rotation", TransformProPreferences.SelectedClipboard), TransformProPreferences.Clipboard.Rotation);
            TransformProPreferences.SetVector3(string.Format("clipboard{0}.Scale", TransformProPreferences.SelectedClipboard), TransformProPreferences.Clipboard.Scale);
        }
        ...
        private static void OnPreferencesGUI()
        {
            ...
            if (TransformProPreferences.installationProblems == true)
            {
                ...
                GUILayout.Label("Conflicting asset detected");
            ...
            TransformProPreferences.EnableBounds = TransformProPreferences.DrawPreferencesField("Enable Calculations", TransformProPreferences.EnableBounds);
            GUI.enabled = TransformProPreferences.EnableBounds;
            TransformProPreferences.BoundsDisplayMode = (BoundsDisplayMode) TransformProPreferences.DrawPreferencesField("Bounds Display Mode", TransformProPreferences.BoundsDisplayMode);
            TransformProPreferences.MaximumBoundsDrawn = Mathf.Max(1, TransformProPreferences.DrawPreferencesField("Maximum Bounds Drawn", TransformProPreferences.MaximumBoundsDrawn));
            GUI.enabled = true;

            // ------------------------------------------------------------------------------------------------------------------------------------------
            GUILayout.Space(5);

            EditorGUILayout.LabelField("Nudges", TransformProStyles.LabelHeading2, GUILayout.Height(24));

            EditorGUI.BeginChangeCheck();

            TransformProPreferences.NudgesPosition.Count = EditorGUILayout.IntSlider("Position", TransformProPreferences.NudgesPosition.Count, 1, 5);
            for (int nudge = 0; nudge < TransformProPreferences.NudgesPosition.Count; nudge++)
            {
                EditorGUILayout.BeginHorizontal();
                TransformProPreferences.NudgesPosition[nudge] = EditorGUILayout.FloatField(string.Format("Nudge {0}", nudge + 1), TransformProPreferences.NudgesPosition[nudge]);
                GUI.enabled = false;
            ...
            TransformProPreferences.NudgesRotation.Count = EditorGUILayout.IntSlider("Rotation", TransformProPreferences.NudgesRotation.Count, 1, 5);
            for (int nudge = 0; nudge < TransformProPreferences.NudgesRotation.Count; nudge++)
            {
                EditorGUILayout.BeginHorizontal();
                TransformProPreferences.NudgesRotation[nudge] = EditorGUILayout.FloatField(string.Format("Nudge {0}", nudge + 1), TransformProPreferences.NudgesRotation[nudge]);
                GUI.enabled = false;
        ...
        public abstract class NudgeSettings : IEnumerable<float>
        {
            ...
            {
                return TransformProPreferences.GetFloat(string.Format("{0}.nudge{1}", this.Name, index), this.GetDefaultValue(index));
            }
        }
        ...
        {
            /// <inheritdoc />
            protected override string Name { get { return "nudgesPosition"; } }

            /// <inheritdoc />
            protected override string Units { get { return "Grid Units"; } }
        ...
        public class NudgeSettingsRotation : NudgeSettings
        {
            ...
            /// <inheritdoc />
            protected override string Name { get { return "nudgesRotation"; } }

            /// <inheritdoc />
            protected override string Units { get { return "Degrees"; } }
